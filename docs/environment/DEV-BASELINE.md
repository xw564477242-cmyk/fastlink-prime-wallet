...
## UI开发分工与工作流
> 模型分工：ChatGPT负责图纸设计，Codex负责代码施工，Railway仅做部署预览，不参与设计决策。

### 角色权责划分
#### 🟦 ChatGPT（设计蓝图，拥有布局决策权）
职责：
1. 参照Railway‑Demo历史视觉风格，定义整套系统统一设计基调：配色、间距、卡片形态、按钮、排版。
2. 规划每页板块布局，定义页面模块顺序、区块划分。
3. 定义完整交互规则：弹窗触发、加载状态、空数据、错误提示、权限不足展示逻辑。
4. 输出文字版页面原型，写明每个区域展示字段、组件联动关系。
5. 将全部方案整理为标准化开发工单，交付Codex执行。

限制短板：
不做像素级精细调参，不熟悉项目现有封装组件；输出为**方案蓝图**，不是可直接运行的成品代码。

#### 🟩 Codex（落地实现，只有施工权，无产品设计权）
职责：
1. 读取本地仓库已有全局UI组件，复用Card/Modal/Button/表单控件，禁止新建一套独立样式体系。
2. 根据ChatGPT下发的开发工单蓝图，翻译为React‑TSX页面代码。
3. 本地运行调试，完成响应式适配、边距、对齐等细小样式微调。

约束：
**不允许自行构思顶层布局、板块顺序、主要功能分区。**
若对布局方案存在分歧，必须反馈给ChatGPT做裁决，不可私自改动整体结构。

#### 🟨 Railway（仅部署预览）
不参与UI构思、不修改布局。
作用：打包前端代码，输出浏览器预览链接，供人工肉眼验收。

### 标准完整链路
1. 人工提出页面需求
2. ChatGPT输出页面布局、交互规则，生成标准化开发工单
3. Codex基于工单蓝图，复用现有组件编写页面代码，输出Diff
4. ChatGPT做代码评审：校验板块排布、模块顺序是否和工单蓝图保持一致；发生偏离则标记阻断，退回修改
5. Codex提交代码 → Railway部署生成预览链接
6. 人工浏览器肉眼验收：
✅布局、区块与蓝图一致；❌出现排布错乱、区块缺失，则回退，更新工单后重新开发

### 三条硬性流程约束
1. **顶层布局、页面板块顺序、主要功能分区：决定权归属ChatGPT，Codex不得自主设计。**
2. **细小样式微调（边距、字号、圆角、间距）：Codex落地阶段直接处理，无需重新下发工单。**
3. **人工新想法：由人工提出，ChatGPT更新布局蓝图，再下发工单给到Codex实施。**

# 四智能体协作‑执行分工约束（写入docs/environment/DEV‑BASELINE.md）

## 附录‑A 角色权责边界
>目的：杜绝越权执行，构建「总控‑执行」隔离链路，所有任务必须遵从流转顺序，不可跳过环节。

### 1）智能总控（ChatGPT‑本会话）
✅允许执行
1. 对DEV‑VERSION系列工单开展评审，识别风险、排查阻塞点
2. 编制任务方案，输出**可下发的审批指令文本**，写明约束红线、验收条件
3. 审阅执行反馈报告，判定放行/驳回，规划下一阶段工作路径
4. 更新项目基线文档，梳理待办事项与技术债务清单

❌严格禁止
1. **不得直接运行git、gh、GitHub‑CLI等任何仓库操作命令**
2. 不能推送分支、创建PR、合并PR、操作远程仓库资源
3. 禁止发起部署、触发GitHub Actions流水线
4. 不负责落地修改workflow、源码补丁，不生成本地commit快照

### 2）执行引擎（Codex独立会话）
✅允许执行
1. 接收由【智能总控】输出的审批指令，在本机工作空间落地任务
2. 修改项目源码、Workflow配置，生成补丁文件、本地Git快照
3. 执行git/gh系列CLI命令：推送分支、新建PR、合并PR，并且留存本地工件（TSv清单、diff、哈希校验文件、工单md）
4. 执行完毕输出DEV‑VERSION‑NN完整工单报告，连同本地证据材料，回传给智能总控进行复审

❌严格禁止
1. **Codex不得自我评审自身输出结果，不能自己批准任务放行**
2. 无权自行编写审批指令，必须等待总控下发任务文本
3. 不允许私自放宽任务内写明的约束红线

### 3）ChatGPT‑Work会话
✅允许执行
1. 只读查阅项目文档、浏览GitHub网页信息，做资料梳理
❌严格禁止
1. **不再分配任何Git/PR/推送/合并/部署类写操作任务**
2. 禁止调用gh、git命令去操作仓库，仅充当信息查阅工具

## A‑1 标准流转链路（强制顺序，不允许跳步）
1️⃣智能总控分析现状 →输出审批指令
2️⃣复制该段指令，下发到Codex会话执行
3️⃣Codex完成工作，产出DEV‑VERSION‑XX工单以及配套本地证据
4️⃣把执行报告传回智能总控会话复审
5️⃣总控给出评审意见，再生成下一条审批指令，循环推进

> 违规判定标记：
> 如果ChatGPT‑Work直接执行仓库CLI命令、自行合并PR，即为越权，该批次执行只能做完当前动作，后续全部任务切回标准流转链路。

## A‑2 不变的全局红线（不受分工调整影响）
1. B03阻塞解除必须经过专项评审，PR合并不等于启动沙箱部署
2. 任何部署动作、Lovable绑定修改，都要有一条独立的专项审批指令，不能附带在别的工单里面捎带执行
3. 每一份DEV‑VERSION工单都必须写明全部约束、现存阻塞、下一步计划。

可以直接复制粘贴进 `docs/environment/DEV‑BASELINE.md`文档末尾。
