  ...
  return (
    <MobileShell>
      ...
      <div className="px-6 pt-4">
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">{t("mp.tag")}</p>
        ...
        {!scanned ? (
          ...
          >
            ...
            <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{t("mp.scanAlign")}</p>
          </button>
        ) : (
          <div className="mt-6 space-y-4">
            <div className="flex items-center gap-4 rounded-3xl border border-border/60 bg-surface/60 p-5">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{t("mp.merchant")}</p>
                <p translate="no" className="truncate text-base font-semibold">Uniqlo · Ion Orchard</p>
                <p translate="no" className="text-[10px] uppercase tracking-widest text-muted-foreground">Order #FL-88231</p>
              </div>
              <button onClick={() => setScanned(false)} className="text-[10px] font-semibold uppercase text-primary">{t("common.change")}</button>
            </div>

            <div className="rounded-3xl border border-border/60 bg-surface/60 p-5">
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground">{t("mp.order")}</p>
              <p translate="no" className="mt-1 font-display text-3xl font-bold tabular-nums">$128.40</p>
            ...
            <div>
              <p className="mb-2 text-[10px] uppercase tracking-widest text-muted-foreground">{t("mp.funding")}</p>
            ...
            <button className="mb-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-primary py-3 font-display text-sm font-semibold text-primary-foreground shadow-glow">
              <QrCode className="h-4 w-4" /> <span translate="no">{t("mp.confirm", { amt: "128.40" })}</span>
            </button>
          </div>
        )}
      </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between">
      <span translate="no" className="text-[11px] text-muted-foreground">{label}</span>
