  ...
  return (
    <MobileShell>
      ...
      <div className="flex items-center justify-between px-6 pt-2">
        ...
        <Link to="/assets/fiat" className="text-xs font-semibold text-primary">{t("assets.linkFiat")}</Link>
      </div>
      ...
      <div className="px-6 pt-4">
        ...
        <p translate="no" className="mt-2 font-display text-4xl font-bold tabular-nums">$14,324.15</p>
        <p translate="no" className="mt-1 text-xs text-primary">{t("assets.digital.change24h")}</p>
      </div>
      ...
      <div className="mt-6 space-y-2 px-6">
        {assets.map((a) => (
          <div key={a.sym} className="flex items-center gap-3 rounded-2xl border border-border/60 bg-surface/60 p-4">
            <div translate="no" className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-primary/15 font-display text-[10px] font-bold text-primary">
              {a.sym}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <p translate="no" className="mt-0.5 text-[11px] tabular-nums text-muted-foreground">{a.chain}</p>
            </div>
            <div className="text-right">
              <p translate="no" className="text-sm font-semibold tabular-nums">${a.usd}</p>
              <p translate="no" className="text-[10px] tabular-nums text-primary">{a.change}</p>
            </div>
          </div>
        ))}
